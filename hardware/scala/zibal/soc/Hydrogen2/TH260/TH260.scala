...
package zibal.soc.hydrogen2

import spinal.core._
import spinal.core.sim._
import spinal.lib._

import zibal.soc.Hydrogen2
...
object TH260 {
  case class Io() extends Bundle {
    ...
    val gpioStatus = Vec(XilinxCmosIo("K12"), XilinxCmosIo("L13"), XilinxCmosIo("K13"),
                         XilinxCmosIo("G11"))
    val freqCounterAPos = XilinxLvdsInput.Pos("F4").clock(100 MHz)
    val freqCounterANeg = XilinxLvdsInput.Neg("F3").clock(100 MHz)
  }
}


object TH260Board {
  def apply(source: String) = TH260Board(source)
  ...
  def main(args: Array[String]) {
    ...
    val compiled = SimConfig.withConfig(config).withWave.workspacePath(elementsConfig.zibalBuildPath).compile {
      val parameter = Hydrogen2.Peripherals.default()
      val peripherals = parameter.peripherals.asInstanceOf[Hydrogen2.Peripherals]
      baudPeriod = peripherals.uartStd.init.getBaudPeriod()
      clockPeriod = 1000000000 / parameter.sysFrequency.toInt

      val board = TH260Board(args(0))
      board
    }
    args(1) match {
      case "boot" =>
  ...
  case class TH260Top(source: String) extends BlackBox {
    val io = TH260.Io()
  ...
  case class TH260Board(source: String) extends Component {
    val io = new Bundle {
    ...
    top.io.freqCounterAPos.PAD := io.freqCounterA
    top.io.freqCounterANeg.PAD := analogFalse
...
object TH260Top {
  ...
  def main(args: Array[String]) {
    ...
    args(0) match {
      case "prepare" =>
        Hydrogen2.prepare(config, elementsConfig, 100 MHz)
      case _ =>
  ...
  case class TH260Top() extends Component {
    val io = TH260.Io()
    ...
    val freqCounterA = IBUFDS(soc.io_per.freqCounterA.clock)
    io.freqCounterAPos <> freqCounterA
    io.freqCounterANeg <> freqCounterA
